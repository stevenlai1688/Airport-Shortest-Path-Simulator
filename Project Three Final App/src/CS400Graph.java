...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    // Checks if the start and end vertices exist and throws a NoSuchElementException if they don't
    if(vertices.get(start)== null || vertices.get(end)== null) {
      throw new NoSuchElementException();
    }
    // Instantiate priority queue to store different paths
    PriorityQueue<Path> paths = new PriorityQueue<Path>();
    // Instantiate linked list to store vertices that have been visited
    LinkedList<T> verts = new LinkedList<T>();
    // Instantiate priority queue to store paths that reach the end node
    PriorityQueue<Path> finalPaths = new PriorityQueue<Path>();
    // Instantiate a new path to store the current path the method is traversing
    Path currentPath = null;
    // Adds the start data to the visited nodes list
    verts.add(start);
    // Adds a new path that begins with the start node given
    paths.add(new Path(vertices.get(start)));
    // Iterates through all of the possible paths with the starting node
    while (paths.size() != 0) {
      // Sets currentPath to the first path in the priority queue and removes it from the queue
      currentPath = paths.poll();
      // Adds the end node of currentPath to the visited nodes list
      verts.add(currentPath.end.data);
      // Checks if the current path leads to the end node
      if(currentPath.end.data.equals(end)) {
        // Adds the path to the priority queue finalPaths if the path leads to the end node
        finalPaths.add(currentPath);
      }
      // Iterates through the edges leaving of the last node in the currentPath
      for(int i = 0; i < currentPath.end.edgesLeaving.size(); i++) {
        // Checks if the specific edge leads to a node that has not been visited
        if(!verts.contains(currentPath.end.edgesLeaving.get(i).target.data)) {
          // Adds a new path object if that node has not been visited to the priority queue
          paths.add(new Path(currentPath, currentPath.end.edgesLeaving.get(i)));
        }
      }
    }
    // Checks if finalPaths is empty and throws a NoSuchElementException if it does
    if(finalPaths.isEmpty()) {
      throw new NoSuchElementException();
    }
    // Returns the cheapest path in finalPaths
